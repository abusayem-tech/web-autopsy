...
export function CopyButton({
  ...
  return (
    <button
      ...
      className={`shrink-0 rounded border border-zinc-200 px-1.5 py-0.5 text-[10px] font-medium text-zinc-600 hover:bg-zinc-50 ${className}`}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    <span className={`inline-flex max-w-full min-w-0 items-start gap-1.5 ${className}`}>
      {linkable ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className={`min-w-0 break-all text-teal-700 underline-offset-2 hover:underline ${mono ? "font-mono" : ""}`}
          title={href}
        >
          {label}
        </a>
