...
export function LoginForm({ nextPath = "/captures" }: { nextPath?: string }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(nextPath.startsWith("/") ? nextPath : "/captures");
      router.refresh();
    } catch (err) {
    ...
    <div className="flex min-h-dvh flex-col bg-gradient-to-b from-teal-50/60 via-zinc-50 to-zinc-50">
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-4 py-12">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm text-zinc-500">Unsaved browsing never leaves your machine.</p>
        </div>
